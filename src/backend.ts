...
export async function getOneMaison(id: string) {
  try {
...
try {
  const sortedRecords = await pb
    .collection('Maisons')
    .getFullList({ filter: 'surface > 100 && nbSdb >= 2' })
  console.table(sortedRecords)
} catch (e) {
  console.error(e)
}
...
try {
  const sortedRecords = await pb.collection('Maisons').getFullList({ sort: 'nomMaison' })
  console.table(sortedRecords)
} catch (e) {
  console.error(e)
}
